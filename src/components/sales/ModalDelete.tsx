...
export const ModalDeleteSales = ({
  ...
  const { deleteSales } = useSalesStore();

  async function confirmDelete({ id }: { id: any }) {
    const res = await deleteSales(selectedSales.invoice);
    setShowModal(false);
    return Swal.fire({
      icon: "success",
      title: "Delete PselectedSales Successful",
      text: `${selectedSales.invoice}`,
      timer: 1500,
        ...
        <div className="h-[32%] flex justify-end items-end">
          <div className="flex justify-end items-end w-3/5">
            ...
            <button
              ...
              onClick={() => confirmDelete({ id: selectedSales.invoice })}
            >
              Yes
            </button>
          </div>
