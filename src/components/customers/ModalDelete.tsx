...
export const ModalDeleteCustomers = ({
  selectedCustomers,
  setShowModal,
}: {
  selectedCustomers: {
  ...
  const { deleteCustomers } = useCustomersStore();

  async function confirmDelete({ id }: { id: any }) {
    const res = await deleteCustomers(selectedCustomers.customerid);
    setShowModal(false);
    return Swal.fire({
      icon: "success",
      title: "Delete Customers Successful",
      text: `${selectedCustomers.name}`,
      timer: 1500,
        ...
        <div className="h-[32%] flex justify-end items-end">
          <div className="flex justify-end items-end w-3/5">
            ...
            <button
              ...
              onClick={() => confirmDelete({ id: selectedCustomers.name })}
            >
              Yes
            </button>
          </div>
